...
public abstract class MaxTest extends AbstractGremlinProcessTest {
    ...
    public void g_V_foo_max() {
        try {
            ...
            try {
                final BufferedWriter writer = new BufferedWriter(new FileWriter("/tmp/exception.txt", true));
                writer.append("MSG: " + ex.getMessage());
                writer.append('\n');
                final Traversal<Vertex, Number> t = get_g_V_foo_max();
                writer.append("HAS NEXT: " + t.hasNext());
                writer.append('\n');
                t.forEachRemaining(res -> {
                    try {
                        writer.append("" + res);
                        writer.append('\n');
